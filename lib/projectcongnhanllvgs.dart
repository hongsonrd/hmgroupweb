...
class TaskScheduleManager {
  ...
  static bool isGSPosition(String positionName) {
    return positionName.trim().toUpperCase().startsWith('GS');
  }
  // Get tasks for a specific project and date using the lookup system
    ...
    return allTasks.where((task) {
      // Get the actual project name and position from the task's username
      ...
      // Filter: Must match project AND position must start with GS
      if (taskProjectName != projectName) return false;
      if (!isGSPosition(positionName ?? '')) return false;
  ...
  static List<TaskScheduleModel> getTasksForPosition(
    ...
    return projectTasks.where((task) {
      final userMapping = getUserProjectAndPosition(task.username, qrLookups);
      final taskPositionName = userMapping['positionName'];
      
      // Filter: Position must match AND must start with GS
      return taskPositionName == positionName && isGSPosition(taskPositionName ?? '');
    }).toList();
  }
static List<String> getGSPositionsForProject(
    String projectName,
    List<QRLookupModel> qrLookups,
  ) {
    final positions = qrLookups
        ...
        .where((position) => position.trim().isNotEmpty && isGSPosition(position))
        .toSet()
        .toList();
    positions.sort();
    return positions;
 ...
 static List<String> getPositionsForProject(
    ...
    return getGSPositionsForProject(projectName, qrLookups);
  }
  static List<String> getProjectsWithGSPositions(List<QRLookupModel> qrLookups) {
    final projectNames = qrLookups
        .where((lookup) => isGSPosition(lookup.vtvalue))
        .map((lookup) => lookup.bpvalue)
        .where((name) => name.trim().isNotEmpty)
        .toSet()
        .toList();
