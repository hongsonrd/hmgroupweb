...
class _ChamCongThangScreenState extends State<ChamCongThangScreen> {
  ...
  Future<void> _loadPredefinedWorkHoursData() async {
    final url = Uri.parse('https://hmclourdrun1-81200125587.asia-southeast1.run.app/chamcongthangb4/');
    final response = await http.post(
      url,
      body: json.encode({
        'username': widget.username,
  ...
  try {
    ...
    for (final entry in employees.entries) {
      ...
      for (final truongHop in truongHopTypes) {
        ...
        sheetSummary.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: currentRow)).value = nguoiDung;
        sheetSummary.cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: currentRow)).value = truongHop;
      ...
      sheetSummary.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: currentRow)).value = userInfo['TenNhanVien'];
      sheetSummary.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: currentRow)).value = nguoiDung;
      sheetSummary.cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: currentRow)).value = 'SoPhepSuDung';
      ...
      sheetSummary.cell(CellIndex.indexByColumnRow(columnIndex: 4, rowIndex: currentRow)).value = soPhepTotal;
      ...
      sheetSummary.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: currentRow)).value = userInfo['TenNhanVien'];
      sheetSummary.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: currentRow)).value = nguoiDung;
      sheetSummary.cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: currentRow)).value = 'TongCong';
      ...
      sheetSummary.cell(CellIndex.indexByColumnRow(columnIndex: 4, rowIndex: currentRow)).value = tongCongTotal;
    ...
    if (_attendanceData.isNotEmpty) {
      final headers = <String>['MaNhanVien', 'TenNhanVien', ..._attendanceData.first.keys];
      ...
      for (int row = 0; row < _attendanceData.length; row++) {
        ...
        final nguoiDung = record['NguoiDung']?.toString() ?? '';
        final userInfo = userLookup[nguoiDung] ?? {'MaNhanVien': '', 'TenNhanVien': ''};
    ...
    if (_otherCaseData.isNotEmpty) {
      final headers = <String>['MaNhanVien', 'TenNhanVien', ..._otherCaseData.first.keys];
      for (int i = 0; i < headers.length; i++) {
    ...
    if (_predefinedWorkHoursData.isNotEmpty) {
      final headers = <String>['MaNhanVien', 'TenNhanVien', ..._predefinedWorkHoursData.first.keys];
      for (int i = 0; i < headers.length; i++) {
