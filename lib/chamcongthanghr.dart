...
class ChamCongThangHRScreen extends StatefulWidget {
  ...
  @override
  _ChamCongThangHRScreenState createState() => _ChamCongThangHRScreenState();
}

class _ChamCongThangHRScreenState extends State<ChamCongThangHRScreen> {
...
List<Map<String, dynamic>> _payStandardData = [];
...
List<String> _getAvailableBranchesFromData() {
  final branches = <String>{};
  
  // Get branches from attendance data
  for (final record in _attendanceData) {
    // Look for branch info in various possible fields
    final branch = _extractBranchFromRecord(record);
    if (branch.isNotEmpty) {
      branches.add(branch);
    }
  }
  
  // Get branches from other case data
  ...
  // Get branches from predefined work hours data
  for (final record in _predefinedWorkHoursData) {
    final branch = record['bophan']?.toString() ?? '';
    if (branch.isNotEmpty) {
      branches.add(branch);
    }
  }
...
String _extractBranchFromRecord(Map<String, dynamic> record) {
  // Try different possible field names for branch information
  final possibleFields = ['branch', 'chinhanh', 'chiNhanh', 'bophan', 'department'];
...
bool _matchBranchNames(String standardBranch, String selectedBranch) {
  if (standardBranch == selectedBranch) return true;
  // Handle common branch name variations
  final branchMappings = {
    'MIENBAC': ['HANOI', 'MIENBAC'],
    'HANOI': ['MIENBAC', 'HANOI'],
    'MIENNAM': ['HCM', 'MIENNAM'],
    'HCM': ['MIENNAM', 'HCM'],
    'SANXUAT': ['SANXUAT', 'PRODUCTION'],
    'LYCHEE': ['LYCHEE'],
    'MIENTRUNG': ['MIENTRUNG', 'DANANG'],
    'DANANG': ['MIENTRUNG', 'DANANG'],
  };
  final mappedNames = branchMappings[standardBranch] ?? [standardBranch];
  return mappedNames.contains(selectedBranch);
}
bool _isValidPeriodForPayroll() {
  ...
  try {
    ...
    // Can create for current month
    if (selectedMonth == currentMonth) return true;
    
    // Can create within 10 days after month ends
    if (selectedMonth.isBefore(currentMonth)) {
      final daysSince = now.difference(DateTime(selectedMonth.year, selectedMonth.month + 1)).inDays;
      return daysSince <= 10;
    }
  ...
  setState(() {
    ...
    if (_adminUsers.contains(widget.username)) {
      ...
      // Add branches based on the user branch map AND available data
      final allBranches = <String>{};
      _userBranchMap.values.forEach((branches) {
        allBranches.addAll(branches);
      });
      
      // Also add any branches found in the actual data if user has access
      if (_hasData) {
        final dataBranches = _getAvailableBranchesFromData();
        for (final branch in dataBranches) {
          // Check if admin has access to this branch type
          for (final branchGroup in _userBranchMap.keys) {
            if (_matchBranchNames(branch, branchGroup)) {
              allBranches.add(branchGroup);
            }
          }
        }
      }
...
void _checkPayStandardAvailability() {
  if (_selectedMonth == null || !_hasData) {
    setState(() {
      _payStandardData = [];
      _canCreatePayroll = false;
    });
    return;
  }
  ...
  final matchingStandards = <Map<String, dynamic>>[];
  
  for (final standard in widget.standardData) {
    try {
      final standardDateString = standard['giaiDoan']?.toString() ?? '';
      if (standardDateString.isEmpty) continue;
      
      final standardDate = DateTime.parse(standardDateString);
      final fixedDate = standardDate.add(const Duration(days: 1));
      final standardMonth = DateTime(fixedDate.year, fixedDate.month);
      
      if (standardMonth == targetMonth) {
        matchingStandards.add(standard);
      }
    } catch (e) {
      // ignore
    }
  ...
  setState(() {
    _payStandardData = matchingStandards;
    _canCreatePayroll = _isValidPeriodForPayroll() && _payStandardData.isNotEmpty;
  });
}
...
Future<void> _showCreatePayrollConfirmation() async {
  if (!_isValidPeriodForPayroll()) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Không thể tạo lịch sử lương cho thời gian này'),
        backgroundColor: Colors.red,
      ),
    );
    return;
  ...
  if (_payStandardData.isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Bạn phải tạo công chuẩn trước khi tạo lịch sử lương'),
  ...
  final confirm = await showDialog<bool>(
    context: context,
    builder: (context) => AlertDialog(
      title: const Text('Tạo lịch sử lương tháng'),
      content: Text(
        'Bạn có muốn tạo lịch sử lương cho tháng $_selectedMonth - $_selectedBranch?\n\n'
        'Quá trình này sẽ tính toán dữ liệu từ chấm công và tạo bản ghi lương.'
      ),
      actions: [
        TextButton(
          onPressed: () => Navigator.of(context).pop(false),
        ...
        ElevatedButton(
          onPressed: () => Navigator.of(context).pop(true),
          child: const Text('Tạo lịch sử lương'),
        ),
      ],
    ),
  );
...
Future<void> _createPayrollData() async {
  showDialog(
    context: context,
    barrierDismissible: false,
    builder: (context) => const AlertDialog(
      content: Column(
        ...
        children: [
          CircularProgressIndicator(),
          SizedBox(height: 16),
          Text('Đang tạo dữ liệu lịch sử lương...'),
        ],
      ),
    ),
  );
  ...
  try {
    final payrollRecords = await _calculatePayrollData();
    Navigator.of(context).pop(); // Close loading dialog
    
    // Navigate to PayHistoryScreen and check for existing records
    _navigateToPayHistory(payrollRecords);
    
  } catch (e) {
    Navigator.of(context).pop();
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Lỗi khi tạo dữ liệu: $e'),
        backgroundColor: Colors.red,
      ),
    );
  }
...
Future<List<Map<String, dynamic>>> _calculatePayrollData() async {
  final userLookup = _buildUserLookup();
  final selectedDateTime = DateFormat('yyyy-MM').parse(_selectedMonth!);
  final daysInMonth = DateTime(selectedDateTime.year, selectedDateTime.month + 1, 0).day;
  ...
  for (final entry in employees.entries) {
    ...
    double tongCongTotal = 0;
    double diMuonTotal = 0;
    double truTienDiMuonTotal = 0;
    double tangCaTotal = 0;
    ...
    for (int day = 1; day <= daysInMonth; day++) {
      ...
      final chamCong = _calculateChamCong(nguoiDung, dateStr, date.weekday);
      final vangNghi = _calculateVangNghi(nguoiDung, dateStr);
      final tangCa = _calculateTangCa(nguoiDung, dateStr);
      final diMuon = _calculateDiMuon(nguoiDung, dateStr);
      
      // Get max work hours for this day
      final maxValue = _getMaxWorkHours(nguoiDung, date.weekday);
      
      // Calculate final TongCong for this day
      final combined = chamCong + vangNghi;
      final finalTongCong = combined > maxValue ? maxValue : combined;
      
      tongCongTotal += finalTongCong;
      diMuonTotal += diMuon;
      tangCaTotal += tangCa;
      
      // Calculate penalty for DiMuon
      double penalty = 0;
      if (diMuon >= 3 && diMuon < 5) {
        penalty = 30000;
      } else if (diMuon >= 5 && diMuon < 10) {
        penalty = 50000;
      } else if (diMuon >= 10 && diMuon < 30) {
        penalty = 70000;
      } else if (diMuon >= 30) {
        penalty = 100000;
      }
      truTienDiMuonTotal += penalty;
    }
    
    // Create payroll record
    final payrollRecord = {
      'uid': '${nguoiDung}_${_selectedMonth}', // Auto-generated UID
      'giaiDoan': DateFormat('yyyy-MM-dd').format(DateTime(selectedDateTime.year, selectedDateTime.month, 1)),
      'userId': nguoiDung,
      'phanNhom': userInfo['BoPhan'] ?? '',
      'maNhanVien': userInfo['MaNhanVien'] ?? '',
      'tenNhanVien': userInfo['TenNhanVien'] ?? '',
      'soTaiKhoan': '', // Blank for now
      'tongCongGoc': tongCongTotal,
      'phutDiMuonGoc': diMuonTotal.toInt(),
      'truDiMuonGoc': truTienDiMuonTotal.toInt(),
      'tongTangCaGoc': tangCaTotal,
    };
...
void _navigateToPayHistory(List<Map<String, dynamic>> payrollRecords) {
  Navigator.push(
    context,
    MaterialPageRoute(
      builder: (context) => PayrollCreationScreen(
        username: widget.username,
...
Widget build(BuildContext context) {
  return Scaffold(
    appBar: AppBar(
      title: const Text('Tổng hợp công tháng HR'),
    ...
    body: Padding(
      ...
      child: Column(
        children: [
                    ...
                    child: DropdownButtonFormField<String>(
                      ...
                      onChanged: (String? newValue) {
                        setState(() {
                          ...
                          _hasData = false;
                          _canCreatePayroll = false; // Reset payroll status
                        });
                      },
                    ),
                  ),
          ...
          // Add the payroll creation status card
          _buildPayrollCreationCard(),
...
Widget _buildPayrollCreationCard() {
  String statusText = '';
  ...
  if (_selectedMonth == null) {
    statusText = 'Chọn tháng để kiểm tra công chuẩn';
    statusColor = Colors.grey;
    statusIcon = Icons.info_outline;
  } else if (!_hasData) {
    statusText = 'Đồng bộ dữ liệu trước';
    statusColor = Colors.blue;
    statusIcon = Icons.sync;
  } else if (!_isValidPeriodForPayroll()) {
    statusText = 'Chỉ tạo được cho tháng hiện tại hoặc trong 10 ngày sau khi tháng kết thúc';
    statusColor = Colors.orange;
    statusIcon = Icons.schedule;
  } else if (_payStandardData.isEmpty) {
    statusText = 'Không có công chuẩn cho tháng $_selectedMonth';
    statusColor = Colors.red;
    statusIcon = Icons.warning;
  } else {
    statusText = 'Có ${_payStandardData.length} bản ghi công chuẩn cho tháng $_selectedMonth';
    ...
    child: Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.monetization_on, color: statusColor, size: 24),
              const SizedBox(width: 8),
              Text(
                'Tạo lịch sử lương tháng',
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Expanded(
                child: Text(statusText, style: TextStyle(fontSize: 14, color: Colors.grey[700])),
              ),
            ],
          ),
          const SizedBox(height: 16),
          Row(
            ...
            children: [
              ElevatedButton.icon(
                onPressed: canCreate ? _showCreatePayrollConfirmation : null,
                icon: Icon(canCreate ? Icons.add : statusIcon, size: 18),
                label: Text(canCreate ? 'Tạo lịch sử lương' : 'Không khả dụng'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: canCreate ? Colors.green : Colors.grey[300],
                  foregroundColor: canCreate ? Colors.white : Colors.grey[600],
                ),
              ),
            ],
          ),
