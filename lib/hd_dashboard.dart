...
class _HDDashboardState extends State<HDDashboard> with TickerProviderStateMixin {
  ...
  Future<void> _loadContractStats() async {
    ...
    // Log first few contracts to see the data structure
    for (int i = 0; i < (contracts.length > 5 ? 5 : contracts.length); i++) {
      final contract = contracts[i];
      print("=== Contract ${i + 1} ===");
      print("ID: ${contract.uid}");
      print("TenHopDong: ${contract.tenHopDong}");
      print("Thang: ${contract.thang}");
      print("DoanhThuDangThucHien: ${contract.doanhThuDangThucHien}");
      
      String extractedPeriod = _extractPeriodFromThang(contract.thang);
      print("Extracted period: '$extractedPeriod'");
      print("================");
    }

    // Group contracts by period and calculate stats
    ...
    for (var contract in contracts) {
      ...
      if (period.isNotEmpty) {
        ...
        // Revenue calculation (DoanhThuDangThucHien)
        double revenue = _safeToDouble(contract.doanhThuDangThucHien);
        periodData[period]!['totalRevenue'] += revenue;
        
        // Total costs calculation (sum of all ChiPhi fields)
        double totalCosts = _calculateTotalCosts(contract);
        periodData[period]!['totalCosts'] += totalCosts;
        
        // Net profit = Revenue - Costs
        periodData[period]!['netProfit'] = 
            periodData[period]!['totalRevenue'] - periodData[period]!['totalCosts'];
        
        print("Contract ${contract.tenHopDong}: Period=$period, Revenue=$revenue, Costs=$totalCosts");
      } else {
    ...
    periodData.forEach((period, data) {
      print("Period $period: ${data['contractCount']} contracts, Revenue: ${data['totalRevenue']}, Costs: ${data['totalCosts']}, Profit: ${data['netProfit']}");
    });
    ...
    for (var stat in sortedStats) {
      print("Period ${stat['period']}: ${stat['contractCount']} contracts, Revenue: ${stat['totalRevenue']}, Costs: ${stat['totalCosts']}, Profit: ${stat['netProfit']}");
    }
  ...
  // Helper method to calculate total costs for a contract
  ...
  return SingleChildScrollView(
    ...
    child: Column(
      children: [
        // You can add other statistical content here or leave it empty
        Center(
          child: Text(
                  ...
                  return ListTile(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        ...
                        Text('Doanh thu: ${_formatCurrency(_safeToDouble(contract.doanhThuDangThucHien))}'),
