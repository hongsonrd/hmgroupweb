...
import 'hd_thangexcel2.dart';

class HDThang2Screen extends StatefulWidget {
  final String period;
  ...
  @override
  _HDThang2ScreenState createState() => _HDThang2ScreenState();
}

class _HDThang2ScreenState extends State<HDThang2Screen> {
  ...
  // Summary data for the period (non-financial only)
    ...
    filtered.sort((a, b) {
      ...
      switch (_sortBy) {
        case 'Tên hợp đồng':
          ...
          break;
        case 'Ngày kết thúc':
          String dateA = a.thoiHanKetthuc ?? '';
          String dateB = b.thoiHanKetthuc ?? '';
          comparison = dateA.compareTo(dateB);
          break;
        case 'Số hợp đồng':
          comparison = (a.soHopDong ?? '').compareTo(b.soHopDong ?? '');
          break;
      }
...
if (contract.uid != null) ...[
  ...
  SizedBox(height: 16),
                ...
                _buildDetailSection('Thông tin ca làm việc', [
                  ...
                  _buildDetailRow('Ghi chú bố trí nhân sự', contract.congNhanGhiChuBoTriNhanSu),
                ]),
                SizedBox(height: 16),
                ...
                if (contract.uid != null) ...[
                  _buildDetailSection('Thông tin hệ thống', [
                    ...
                    _buildDetailRow('File hợp đồng', '${contract.fileHopDong ?? 'N/A'}'),
        ...
        return ListTile(
          ...
          subtitle: Column(
            ...
            children: [
              ...
              Text('CN được có: ${contract.congNhanDuocCo?.toString() ?? 'N/A'}'),
          ...
          rows: _filteredContracts.map((contract) {
            return DataRow(
              cells: [
               ...
               DataCell(
                 Text(contract.soHopDong ?? 'N/A'),
                 onTap: () => _showContractDetails(contract),
               ),
               DataCell(
                 Text(
                   contract.congNhanDuocCo?.toString() ?? 'N/A',
                   style: TextStyle(
                     color: Colors.blue,
                     fontWeight: FontWeight.bold,
                   ),
