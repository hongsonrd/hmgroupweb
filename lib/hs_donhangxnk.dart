...
class _HSDonHangXNKScreenState extends State<HSDonHangXNKScreen> {
  ...
  Future<void> _createXNKOrder() async {
    ...
    // Create the main DonHang object for XNK
    _newOrder = DonHangModel(
      soPhieu: soPhieu,
      nguoiTao: userCredentials.username,
      ngay: formattedDate,
      tenKhachHang: 'XNK Đặt hàng tháng $currentMonth',
      sdtKhachHang: '', // Default empty
      soPO: '', // Default empty
      diaChi: '', // Default empty
      mst: '', // Default empty
      tapKH: 'KH Truyền thống', // Default
      tenNguoiGiaoDich: '', // Default empty
      boPhanGiaoDich: '', // Default empty
      sdtNguoiGiaoDich: '', // Default empty
      ...
      datCocSauXNgay: 0, // Default
      giayToCanKhiGiaoHang: '', // Default empty
      thoiGianVietHoaDon: 'Hoá đơn viết ngay khi giao hàng', // Default
      thongTinVietHoaDon: '', // Default empty
      diaChiGiaoHang: '', // Default empty
      hoTenNguoiNhanHoaHong: '', // Default empty
      sdtNguoiNhanHoaHong: '', // Default empty
      hinhThucChuyenHoaHong: '', // Default empty
      thongTinNhanHoaHong: '', // Default empty
      ngaySeGiao: '', // Default empty
      thoiGianCapNhatMoiNhat: currentDateTime,
      phuongThucGiaoHang: 'HMGROUP', // Retained
      phuongTienGiaoHang: '', // Default empty
      hoTenNguoiGiaoHang: '', // Default empty
    ...
    for (DonHangModel selectedOrder in _selectedDutruOrders) {
      ...
      for (ChiTietDonModel originalItem in originalItems) {
        final newChiTiet = ChiTietDonModel(
          ...
          soLuongKhachNhan: originalItem.soLuongKhachNhan,
          duyet: 'Chưa xong',
          xuatXuHangKhac: originalItem.xuatXuHangKhac,
  ...
  Future<void> _submitOrder() async {
    ...
    try {
      ...
      final orderData = {
        ...
        'DiaChiGiaoHang': _newOrder?.diaChiGiaoHang,
        'HoTenNguoiNhanHoaHong': _newOrder?.hoTenNguoiNhanHoaHong,
        //'SDTNguoiNguoiNhanHoaHong': _newOrder?.sdtNguoiNhanHoaHong, // Corrected field name
        'HinhThucChuyenHoaHong': _newOrder?.hinhThucChuyenHoaHong,
        'ThongTinNhanHoaHong': _newOrder?.thongTinNhanHoaHong,
        'NgaySeGiao': _newOrder?.ngaySeGiao,
        'ThoiGianCapNhatMoiNhat': _newOrder?.thoiGianCapNhatMoiNhat,
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        ...
                        subtitle: Text('${item.baoGia != null && item.baoGia!.isNotEmpty ? '[${item.baoGia}] ' : ''}${item.soLuongYeuCau} ${item.donViTinh} x ${currencyFormat.format(item.donGia)} VNĐ'),
                        trailing: IconButton(
  ...
  // Renamed _buildOrderDetailsStep for clarity, but its content is largely removed
  // as order details are now mostly automated.
  // This method would be skipped in the new flow.
  // If you still need a way to edit the new order's header details,
  // you might introduce a new "Step 1" for that, but based on the prompt,
  // it seems to be automated.
  // I will omit the content of this method for now, as it's no longer used.
  // You can decide if you want to repurpose it or remove it entirely.
